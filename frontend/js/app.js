angular.module("expo", [])
  .controller("HeatmapController", function($scope, $interval,
    UserLocationService) {
    $scope.heatmap = {};
    $scope.heatmap.ratio = {};
    $scope.heatmap.poi = {
    ...
    $scope.getUserCoordinates = function() {
      UserLocationService.getCoordinates($scope.heatmap.ratio.rx, $scope.heatmap
          .ratio.ry)
        .then(function(data) {
  ...
  .service("UserLocationService", function($http) {
    var basckendServiceHost = "http://localhost/explore/server/public/";
    ...
    this.getCoordinates = function(rx, ry) {
      ...
      return $http({
        method: 'GET',
        url: basckendServiceHost + 'api/users/coordinates?rx=' + rx +
          '&ry=' + ry
      });
    }
    this.getHeatmapConfig = function() {
      return $http({
        method: 'GET',
        url: basckendServiceHost + 'api/floors'
      });
    }
      ...
      controller: function($scope, UserLocationService) {
        ...
        this.configHeatmap = function(htmlElem) {
          ...
          UserLocationService.getHeatmapConfig().then(function(data) {
            $(htmlElem[0]).css("background-image", "url(" + data.data[
              0].image + ")");
            $scope.heatmap.ratio.rx = htmlElem[0].clientWidth / data.data[
              0].width;
            $scope.heatmap.ratio.ry = htmlElem[0].clientHeight / data
              .data[0].length;
